...
def api_profile():
    ...
    if data.get("email"):
        cur.execute("UPDATE users SET email=? WHERE id=?", (data["email"], user["id"]))

    if data.get("password"):
        cur.execute("UPDATE users SET password_hash=? WHERE id=?", (generate_password_hash(data["password"]), user["id"]))
